const router = require('express').Router();
const { Tag, Product, ProductTag } = require('../../models');

// The `/api/tags` endpoint

router.get('/', async (req, res) => {
  // find all tags
  // be sure to include its associated Product data
  try {
    const tags = await Tag.findAll({ include: ({model: Product, ProductTag}) });
    res.status(200).json(tags)
  }
  catch (err) {
    res.status(500).json({ message: 'tag not found'})
  }
});

router.get('/:id', async (req, res) => {
  // find a single tag by its `id`
  // be sure to include its associated Product data
  try {
     const oneTag = await Tag.findByPk( req.params.id, { include: ({ model: Product, ProductTag}) })
     if(!oneTag) {
      res.status(404).json({ message: `no tag with the id of '${req.params.id}'`})
      return;
     }
     res.status(200).json(oneTag)
  } catch (err) {
    res.status(500).json({ message: 'id not found'})
  }
});

router.post('/', async (req, res) => {
  // create a new tag
  try{ 
    const newTag = await Tag.create(req.body)
    res.status(200).json(newTag)
  } catch (err) {
    res.status(500).json('tag not created')
  }
});

router.put('/:id', async (req, res) => {
  // update a tag's name by its `id` value
  try {
  const updateTag = await Tag.update( req.body, { where: { id: req.params.id}})
  if(!updateTag[0]) {
    res.status(404).json({message: `no tag with the id of '${req.params.id}'`})
  } res.status(200).json(updateTag)
  } catch (err) {
    res.status(500).json('tag not updated')
  }
});

router.delete('/:id', async (req, res) => {
  // delete on tag by its `id` value
  try {
    const deletedTag = await Tag.destroy({where: { id: req.params.id}})
    !deletedTag ? res.status(404).json({ message:`no tag with the id of '${req.params.id}'`})
    : res.status(200).json(deletedTag)
  } catch (err) {
    res.status(500).json(err)
  }
});

module.exports = router;
